(function () {
    ...
    var deviceScaleMap = {};
    // The PI receives a didReceiveSettings echo every time WE call setSettings (e.g.
    // every keystroke or add/remove). If we naively rebuild rows from settings on every
    // echo, the user's just-added empty row gets wiped before they can type anything.
    // Render rows from settings exactly once — initial load — then trust the DOM.
    var presetsRenderedOnce = false;
    ...
    function updateAddButton() {
        ...
        // Keep the hint text in sync with the live cap so HTML and JS can't drift.
        var hint = document.getElementById('presetMaxHint');
        if (hint) {
            hint.textContent = 'Up to ' + MAX_PRESETS;
        }
    }
    ...
    function wrapLoadConfiguration() {
        ...
        loadConfiguration = function (payload, isglobal) {
            ...
            if (!isglobal) {
                ...
                // Only render rows from settings on the FIRST receive. Subsequent
                // didReceiveSettings events are echoes of our own setSettings calls
                // and rebuilding would wipe the user's in-progress empty row.
                if (!presetsRenderedOnce) {
                    loadFromHidden();
                } else {
                    updateValidation();
                    updateAddButton();
