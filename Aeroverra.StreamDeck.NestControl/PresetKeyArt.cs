...
namespace Aeroverra.StreamDeck.NestControl
{
    ...
    internal static class PresetKeyArt
    {
        ...
        private static string BuildPositionIndicator(int selectedIndex, int presetCount)
        {
            ...
            // Fallback to a textual N/M indicator only for absurd counts we don't expect to
            // see in practice (cap is 10 in PresetConfiguration).
            if (presetCount > 12)
            {
            ...
            // Adapt spacing/radius so up to 10 dots fit cleanly inside the 72px key without
            // touching the edges.
            var spacing = presetCount <= 6 ? 7.0 : presetCount <= 8 ? 6.0 : 5.5;
            var selectedRadius = presetCount <= 6 ? 2.8 : presetCount <= 8 ? 2.4 : 2.1;
            var unselectedRadius = presetCount <= 6 ? 2.0 : presetCount <= 8 ? 1.7 : 1.5;
            ...
            for (var i = 0; i < presetCount; i++)
            {
                ...
                var radius = i == selectedIndex ? selectedRadius : unselectedRadius;
                dots.Append(FormattableString.Invariant(
                    $"""<circle cx="{x:F1}" cy="{y}" r="{radius:F1}" fill="#FFFFFF" fill-opacity="{opacity:F2}"/>"""));
            }
